...
contract('UniversalResolver', function () {
  ...
  describe('encodeName()', () => {
    it('should encode a name', async () => {
      const result = await testNameEncoder.encodeName('foo.astra')
      expect(result['0']).to.equal(dns.hexEncodeName('foo.astra'))
      expect(result['1']).to.equal(namehash('foo.astra'))
    })
    ...
    it('should encode a long name', async () => {
      const result = await testNameEncoder.encodeName(
        'something.else.test.astra',
      )
      expect(result['0']).to.equal(
        dns.hexEncodeName('something.else.test.astra'),
      )
      expect(result['1']).to.equal(namehash('something.else.test.astra'))
    })
  })
})
