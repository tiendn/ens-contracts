...
async function main(a) {
  ...
  await (
    await Resolver.setText(
      ...
      'https://i.imgur.com/JcZESMp.png',
    )
  ).wait()
  console.log(
    `Wrapped NFT for sub2.${name}.astra is available at ${getOpenSeaUrl(
      NameWrapper.address,
      namehash(`sub2.${name}.astra`),
    )}`,
  )
  await (
    await NameWrapper.burnFuses(namehash(`sub2.${name}.astra`), CANNOT_UNWRAP)
  ).wait()
  await (
